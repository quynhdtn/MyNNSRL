...
from keras.layers import Dense, Dropout, Activation, Reshape, TimeDistributedDense, Masking, RepeatVector
...
class S2SModel(Model):
    def __init__(self, input_dim,maxlen, lstm_size, output_dim):
     ...
     #   model.add(Embedding(max_features, 128, input_length=maxlen))
        model.add(LSTM(lstm_size, input_dim=input_dim))  # try using a GRU instead, for fun
